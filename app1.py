...
class FireDetectionSystem:
    ...
    def create_alarm_sound(self):
        ...
        # Platform-specific alarm initialization
        if platform.system() == 'Darwin':  # macOS
            # macOS: Use system alert sounds
            self.alarm_file = "/System/Library/Sounds/Sosumi.aiff"
            if not os.path.exists(self.alarm_file):
                self.alarm_file = "/System/Library/Sounds/Ping.aiff"
            
            # Try to create voice alert on macOS
            ...
            # Windows: Use winsound module for beeping
            self.alarm_file = 'winsound'
            self.voice_file = None
            print("Windows alarm system initialized (using system beep)")
            
        else:  # Linux and others
            # Linux: No reliable built-in alarm
            self.alarm_file = None
            self.voice_file = None
            ...
            if platform.system() == 'Windows':
                try:
                    ...
                    for _ in range(3):
                        ...
                        time.sleep(0.3)
                        winsound.Beep(1500, 1000)  # Higher pitch, longer
                        time.sleep(0.5)
                except ImportError:
                    print("Warning: winsound module not available")
                    print("\a\a\a")  # Terminal bell
                
            # macOS: Use afplay for alarm sounds and voice
                ...
                # Linux and others: Use terminal bell
                print("\a\a\a")  # Terminal bell
                print("WARNING: Using terminal bell as fallback alarm")
                time.sleep(0.5)
                print("\a\a\a")
    ...
    def send_notification(self, image_path=None):
        ...
        try:
            ...
            if platform.system() == 'Darwin':  # macOS
                ...
                # Use Windows notification via Python's built-in methods
                try:
                    import tkinter
